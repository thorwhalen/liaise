...
message send` opens none (liaise #28). It is an [`OutboundMessage`](liaise.model.md#liaise.model.OutboundMessage) in
...
- [`send_message()`](#liaise.messages.send_message) takes a GitHub issue, or a repository and a title to open an issue,
  ...
  ([`liaise.subjects.subject_for_ref()`](liaise.subjects.md#liaise.subjects.subject_for_ref)), so the caller cannot pick a laxer policy. The
...
- [`send_held_message()`](#liaise.messages.send_held_message) is how the operator sends a held one, through
  [`liaise.release.release_draft()`](liaise.release.md#liaise.release.release_draft), as a case’s draft is sent.
...
| [`message_draft`](#liaise.messages.message_draft)(message)                             | `message` as a held draft: the shape [`liaise.release.release_draft()`](liaise.release.md#liaise.release.release_draft) releases. |
...
| [`MessageRelease`](#liaise.messages.MessageRelease)(draft, attempt, filters, ...)   | What [`send_held_message()`](#liaise.messages.send_held_message) did, as [`liaise.cases.DraftRelease`](liaise.cases.md#liaise.cases.DraftRelease) does for a case.   |
|-------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
What [`send_held_message()`](#liaise.messages.send_held_message) did, as [`liaise.cases.DraftRelease`](liaise.cases.md#liaise.cases.DraftRelease) does for a case.
...
`message` as a held draft: the shape [`liaise.release.release_draft()`](liaise.release.md#liaise.release.release_draft) releases.
...
[`MESSAGE_STATES`](liaise.model.md#liaise.model.MESSAGE_STATES).
...
([`liaise.cases.held_lines()`](liaise.cases.md#liaise.cases.held_lines)); and its `entries` latest entries. Reads only.
...
It goes out through [`liaise.release.release_draft()`](liaise.release.md#liaise.release.release_draft) with `by`’s approval on the
context, as a case’s draft does ([`liaise.cases.send_draft()`](liaise.cases.md#liaise.cases.send_draft)), with `text` and
...
since `seen`, and anything [`release_draft()`](liaise.release.md#liaise.release.release_draft) refuses.
...
[`sendable_ref()`](liaise.release.md#liaise.release.sendable_ref) gives it. Its subject is the one
[`subject_for_ref()`](liaise.subjects.md#liaise.subjects.subject_for_ref) names. The message is judged by the gate
...
Raises [`DraftSentNotRecorded`](liaise.release.md#liaise.release.DraftSentNotRecorded) when the message went out and
