...
- **Agreement is by flow class.** A message is either sent with no person looking (`send`) or held back (every other flow); 0.1 could say nothing finer, so the policy's `delay`, `revise` or `refuse` agrees with a 0.1 divert. *Shadow agreement* is the share of shadow messages compared where the classes match.
...
- **`mode = "shadow"` enforces like `enforce`, always**, and measures the policy against 0.1: see "Shadow mode is a counterfactual" below (issues #39 and #51).
