...
- **PreToolUse**, on `Bash` and on correspond’s MCP write tools. [`pre_tool_use()`](#liaise.hook.pre_tool_use)
  ...
  ([`liaise.vet.vet()`](liaise.vet.html.md#liaise.vet.vet), with the provenance unknown) and answers `deny` with the
...
- **PostToolUse**, on the same tools. When a command the hook answered `ask` ran, the
  ...
  ([`liaise.ledger.Ledger.add_override()`](liaise.ledger.html.md#liaise.ledger.Ledger.add_override)), never the text.
