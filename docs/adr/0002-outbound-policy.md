...
- **Private words inside a link's path (issue #46).** A term a link spells out as a word of its path, query or fragment was always found, like a term anywhere else. Since issue #46, one glued to other words there (`/HeronTerms.pdf`, `?p=heron2026`, percent-encoded or not) is found too, by the `term-in-link` rule, with the same kind, label, severity and fingerprint, so the table routes it as the term. That moves a message off `approve` only where something else holds it back further (a tainted run refuses it) and lifts a `delay` to `revise` where the host is allowlisted; a clean message linking to an unknown host on a wide audience stays at `approve`, now with the term among the reasons the operator reads. Terms under four characters are not looked for inside a link, where a hash split at its digits would match them by chance. Still missed: a term glued to an all-capitals run (`HERONterms`), which cannot be split without guessing, and a term glued into the host, which only the link-host rule reads.
