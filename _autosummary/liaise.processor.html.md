...
- `start(job)` and `resume(session_id, job)`: spawn a detached run and return its
  [`RunRecord`](liaise.model.html.md#liaise.model.RunRecord) at once. Both are idempotent on `job.run_id`.
...
- `collect(run)`: `None` while the run is going, then its
  [`RunResult`](liaise.model.html.md#liaise.model.RunResult), classified by [`liaise.errors.classify()`](liaise.errors.html.md#liaise.errors.classify).
