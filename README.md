...
**The run's own budget.** The timeout is enforced from outside: past `timeout_minutes`, the tick stops the run and classifies it `timed_out`. The turn cap is a line in the prompt, since `claude -p` has no flag for it.
