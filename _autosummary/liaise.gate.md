...
A processor run reports outcomes, [`liaise.outcomes`](liaise.outcomes.md#module-liaise.outcomes) plans them into actions, and each
[`Send`](liaise.outcomes.md#liaise.outcomes.Send) among those is an [`Outbound`](#liaise.gate.Outbound) that the tick hands to
...
2. [`outbound_policy()`](#liaise.gate.outbound_policy): the policy of liaise discussion 32. Who can read the
   ...
   rule table of [`liaise.policy`](liaise.policy.md#module-liaise.policy). Draft reply mode is a row of that table. It
...
the message must be held back: its `flow`, one of [`liaise.policy.FLOWS`](liaise.policy.md#liaise.policy.FLOWS)
...
the case’s outbox for a cancellable window (liaise #38, [`liaise.outbox`](liaise.outbox.md#module-liaise.outbox)), then judges
...
**Approvals** (discussion §5.7). The operator’s [`Approval`](liaise.model.md#liaise.model.Approval) on
...
([`liaise.policy.payload_hash()`](liaise.policy.md#liaise.policy.payload_hash), [`liaise.policy.audience_hash()`](liaise.policy.md#liaise.policy.audience_hash)), and the name of
what that verdict flagged ([`liaise.outbound.verdict_id()`](liaise.outbound.md#liaise.outbound.verdict_id)). While all three still hold,
...
The gate only decides. [`liaise.release.gate_and_send()`](liaise.release.md#liaise.release.gate_and_send) computes the audience, runs the
gate, and sends `GateDecision.send`; its callers keep a diverted message as a draft
(see [`liaise.outcomes.make_draft()`](liaise.outcomes.md#liaise.outcomes.make_draft)) and record [`GateDecision.record()`](#liaise.gate.GateDecision.record). acquaint is
...
`flow` is one of [`liaise.policy.FLOWS`](liaise.policy.md#liaise.policy.FLOWS) other than `send`; a divert that does
...
liaise 0.1’s gate would have decided about the same message ([`liaise.legacy`](liaise.legacy.md#module-liaise.legacy)),
...
what the verdict flags ([`liaise.outbound.verdict_id()`](liaise.outbound.md#liaise.outbound.verdict_id)), which is `None` when no
...
An [`Approval`](liaise.model.md#liaise.model.Approval) by [`OUTBOX_ACTOR`](#liaise.gate.OUTBOX_ACTOR), bound as an operator’s is
...
[`notify_addresses_for()`](liaise.subjects.md#liaise.subjects.Subject.notify_addresses_for)). A missing mention is
