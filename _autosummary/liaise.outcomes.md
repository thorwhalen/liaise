...
outcomes, each a `kind` from the closed vocabulary [`OUTCOME_KINDS`](liaise.model.md#liaise.model.OUTCOME_KINDS)
with its `text`, `questions` and `reason`.

- [`parse_outcomes()`](#liaise.outcomes.parse_outcomes) checks that output and returns its
  [`Outcome`](liaise.model.md#liaise.model.Outcome) records, or raises `ValueError` listing every problem.
...
the actions in order, and passes each [`Send`](#liaise.outcomes.Send) through [`liaise.gate.run_gate()`](liaise.gate.md#liaise.gate.run_gate)
before sending it.
...
Bases: [`Outbound`](liaise.gate.md#liaise.gate.Outbound)
