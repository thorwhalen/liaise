...
- **shadow**: the judged messages of subjects in `policy.mode = "shadow"`. Shadow mode
  ...
  each verdict the gate records what liaise 0.1 would have decided ([`liaise.legacy`](liaise.legacy.html.md#module-liaise.legacy)).
  **Compared** counts the shadow messages that carry that counterfactual; \*\*shadow
  ...
  ([`liaise.legacy.flow_class()`](liaise.legacy.html.md#liaise.legacy.flow_class): sent, or held back), and a **missed finding** is one
