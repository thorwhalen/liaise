...
[`liaise.model.OUTCOME_KINDS`](liaise.model.md#liaise.model.OUTCOME_KINDS), which must all have an entry.
...
[`brief_for()`](liaise.subjects.md#liaise.subjects.Subject.brief_for)), the case pointer (each conversation, as
...
Raises `ValueError` for an unknown mode, and [`ConfigError`](liaise.config.md#liaise.config.ConfigError)
