...
loaded, validated, shown and used as gate context, but [`liaise.tick.run_once()`](liaise.tick.md#liaise.tick.run_once) polls
...
Like [`liaise.config`](liaise.config.md#module-liaise.config), this module knows only the file’s shape and defaults.
...
> [`liaise.access`](liaise.access.md#module-liaise.access). `waiting_labels` (person to label) is the mirror of
> `claim_labels`: a label liaise writes on a case’s issues while the case waits on that
> person, where a claim label is one it reads (see [`liaise.projection`](liaise.projection.md#module-liaise.projection)).
...
When a case is ready to dispatch (see [`liaise.readiness`](liaise.readiness.md#module-liaise.readiness)).
...
[`ConfigError`](liaise.config.md#liaise.config.ConfigError) naming the file and the fix for a missing file,
