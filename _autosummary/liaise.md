...
| [`hook`](liaise.hook.md#module-liaise.hook)             | The Claude Code hook: every `gh` or `correspond` write from any session is vetted first (discussion 32, §5.8).       |
...
| [`legacy`](liaise.legacy.md#module-liaise.legacy)         | What liaise 0.1's gate would have decided: the counterfactual shadow mode measures against (liaise #39, #51).        |
